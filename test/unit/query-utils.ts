...
async function readFromContract(client: Client, method: string, args: any[], printResult?: boolean): Promise<any> {
    const res = await performQuery(client, method, args);
    if (printResult) logResult(res);
    return res
};

async function execMethod(client: Client, signature: string, method: string, args: any[], printResult?: boolean): Promise<Receipt> {
